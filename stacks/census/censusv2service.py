...
class TableV3Service(DownloadTable, CensusService):
    def build_response(self):
        ...
        results = RecipePool([(recipe1, "States"), (recipe2, "Ages")]).run()
        self.response["responses"] = results
        self.response["responses"][0]["templateContext"][
            "notes"
...
class RankedListV3Service(CensusService):
    def build_renderers(self):
        ...
        return [
            self.renderer("States", data=recipe1),
            self.renderer("Gender", data=recipe2),
        ]
...
class RankedListRawQuery(CensusService):
    ...
    def build_renderers(self):
        ...
        gender_query = (
            select(
                [
                    ...
                    case({"F": "Womenfolk", "M": "Menfolk"}, value=Census.sex).label(
                        "sex"
                    ),
                    avgage_clause.label("avgage"),
                    func.sum(Census.pop2008).label("pop2008"),
        ...
        metadata = {
            "avgage": {"format": ".1f"},
            "pop2008": {"format": ".3s"},
            "state": {"singular": "State", "plural": "State"},
            "sex": {"singular": "sex", "plural": "sex"},
        }
...
class CardV3Service2(CensusService):
    def build_response(self):
        ...
        results = RecipePool([(recipe1, "States"), (recipe2, "Gender")]).run()
